...
class CustomerWalletProvider with ChangeNotifier {
  ...
  Future<Map<String, dynamic>> updateWallet(double amount) async {
    ...
    try {
      ...
      final walletDoc =
          await walletRef.doc(user.uid).set({"amount": totalAmount});
  ...
  Future<Map<String, dynamic>> initiatePaystackTransfer(
      amount, recipient, reason) async {
    http.Response response =
        await http.post(Uri.parse("https://api.paystack.co/transfer"),
            body: json.encode({
              "amount": amount,
              "recipient": recipient,
              "currency": "NGN",
  ...
  Future<Map<String, dynamic>> withdrawFromWallet(
      double amount, String account_bank, account_number, String name) async {
    amount = amount.toDouble();

    if (amount < 1000) {
      return {
        "status": false,
        "message": "Withdrawable wallet amount must be at least 1000 NGN. "
      };
    }
    try {
      var user = FirebaseAuth.instance.currentUser;
      ...
      if (amount > wallet) {
        ...
        return {
          "status": false,
          "message": "Amount to withdraw is more than wallet balance"
        };
      }
      print("reeqData#");
      print({
      ...
      http.Response response = await http
          .post(Uri.parse("https://api.paystack.co/transferrecipient"),
              body: json.encode({
                "type": "nuban",
                "name": name,
                "bank_code": account_bank,
      ...
      if (resp["status"]) {
        Map<String, dynamic> transResp = await initiatePaystackTransfer(
            amount,
            resp["data"]["recipient_code"],
            "Withdrawal from my intecPRO wallet");
...
// }
  ...
  Future<Map<String, dynamic>> initiatePayment(email, amount) async {
    try {
      final http.Response response =
          await http.post(Uri.parse("${base_url}initiate/payment"),
              body: json.encode({
                "email": email,
                "amount": amount,
  ...
  Future<Map<dynamic, dynamic>> initiateWebPayment(email, amount) async {
    try {
      var user = FirebaseAuth.instance.currentUser;
      var tokenRes = await user!.getIdTokenResult();

      final http.Response response = await http.post(
          Uri.parse("${base_url}transaction/initiate"),
          body: json.encode({
            "email": email,
            "amount": amount,
      ...
      return {"status": false, "message": "Failed to make generate payment"};
    } catch (e) {
      print("error payment");
      print(e);
      return {"status": false, "message": "Something went wrong"};
 ...
 Future<bool> verifyPayment(reference) async {
    try {
      var user = FirebaseAuth.instance.currentUser;
      var tokenRes = await user!.getIdTokenResult();

      final http.Response response = await http.post(
          Uri.parse("${base_url}transaction/confirm"),
          body: json.encode({
            "reference": reference,
